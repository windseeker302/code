...
if __name__ == "__main__":
    ...
    try:
        with open("cce_cluster.yaml",encoding='utf-8') as f:
            cce_config = yaml.safe_load(f)
        request = CreateClusterRequest()
        listCidrsContainerNetwork = [
            ContainerCIDR(
                cidr="10.0.0.0/12"
        ...
        hostNetworkSpec = HostNetwork(
            vpc=cce_config['cluster']['vpc_id'],
            subnet=cce_config['cluster']["vpc_sub_id"]
        )
        specbody = ClusterSpec(
            category=cce_config['cluster']['category'],
            type=cce_config['cluster']['type'],
            flavor=cce_config['cluster']['flavor'],
            version=cce_config['cluster']['version'],
            host_network=hostNetworkSpec,
            container_network=containerNetworkSpec
        )
        metadatabody = ClusterMetadata(
            name=cce_config['cluster']['name']
        )
        request.body = Cluster(
            spec=specbody,
            metadata=metadatabody,
