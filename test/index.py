...
HOST = os.getenv("MQTT_HOST") if os.getenv("PROD") else "MQTT_HOST_LOCALHOST"
PORT = 1883
print('生产环境部署' if os.getenv("PROD") else "MQTT_HOST_LOCALHOST")
# 监听字段更新
...
# 当前模型加载路径
SAVE_MODEL_PATH = "./vegetable_model_v3.h5"
...
CLASS_NAMES = ['苹果', '香蕉', '甜菜根', '甜椒', '卷心菜', '灯笼椒', '胡萝卜', '花椰菜', '辣椒', '玉米', '黄瓜', '茄子', '大蒜', '姜', '葡萄', '墨西哥胡椒', '猕猴桃', '柠檬', '生菜', '芒果', '洋葱', '橙子', '红椒', '梨', '豌豆', '菠萝', '石榴', '土豆', '萝卜', '大豆', '菠菜', '甜玉米', '红薯', '番茄', '芜菁', '西瓜']
...
def analyzer():
    ...
    if file:
        try:
            ...
            test_image_src = test_image_src.convert("RGB")
